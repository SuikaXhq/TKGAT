...
workdir = 'trained_model/GraphRec/'

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', default='delicious', help='dataset name: Ciao/Epinions')
	...
	for line in lines:
		...
		if len(inter) > 1:
			...
			for item_id in item_ids:
				result.append([user_id, item_id, 0])
...
elif args.dataset == 'lastfm' or args.dataset == 'delicious':
	click_f_train = load_interaction(f'datasets/{args.dataset}/train.txt')
	click_f_valid = load_interaction(f'datasets/{args.dataset}/valid.txt')
	click_f_test = load_interaction(f'datasets/{args.dataset}/test.txt')
	click_f_train += 1
	click_f_valid += 1
	click_f_test += 1
	trust_f = np.loadtxt(f'datasets/{args.dataset}/social_network.txt', dtype = np.int32)
	trust_f[:, 1] = trust_f[:, 2]
	trust_f[:, 2] = 0
	trust_f += 1
else:
...
if args.dataset == 'lastfm' or args.dataset == 'delicious':
	click_list = np.concatenate([click_f_train, click_f_valid, click_f_test], axis=0)
	user_count = max(click_list[:, 0])
	item_count = max(click_list[:, 1])
	rate_count = 1

	train_set = click_f_train
	valid_set = click_f_valid
	test_set = click_f_test
else:
	for s in click_f:
		uid = s[0]
		iid = s[1]
