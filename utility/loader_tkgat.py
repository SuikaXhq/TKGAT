...
class DataLoaderTKGAT(object):
    ...
    def statistic_cf(self):
        ...
        self.n_users = max(max(self.cf_train_data[0]), max(self.cf_test_data[0])) + 1 # user index start from 0
        self.n_items = max(max(self.cf_train_data[1]), max(self.cf_test_data[1])) + 1 # item index start from 0
    ...
    def create_graph(self, kg_data, n_nodes):
        g = dgl.DGLGraph()
        g.add_nodes(n_nodes)
        g.add_edges(kg_data['h'], kg_data['t'])
        g.readonly()
